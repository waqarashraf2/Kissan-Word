...
<article class="article-card reveal">
    <a href="{{ route($blog->language === 'ur' ? 'blogs.urdu.show' : 'blogs.english.show', $blog->slug) }}" class="article-image">
        @if($blog->featured_image)<img src="{{ asset(ltrim($blog->featured_image, '/')) }}" alt="{{ $blog->featured_image_alt ?: $blog->title }}" loading="lazy" width="700" height="440">@else<span>{{ $blog->language === 'ur' ? 'کسان ورلڈ' : 'KISANWORLD' }}</span>@endif
    </a>
